...
def train_policy_Oracle_no_cf(n_train=2000, seed=20026, max_alt_iters=30, tau=0.5, phi_type=1, model_type="nn", save_models=False, dgp="S1", optim_mode="scl"):
    ...
    print("\n" + "="*50)
    print(f"🚀 Starting Oracle Policy Learning (NO-CF MODE, {dgp})")
    n_val = int(n_train * 0.25)
    params_dgp = origin_para_set
    df_train = data_gen(n_train, params_dgp, scenario=dgp)
    df_val = data_gen(n_val, params_dgp, scenario=dgp)
    
    print("\n=== Oracle Step 1: Estimating Propensity Weights (Oracle Features) - NO CF ===")
    ...
    print("\n=== Oracle Step 2 & 3: Alternating Optimization (Binary Search Version) ===")
