...
def prepare_outer_tensors(df: pd.DataFrame, oracle_weights: np.ndarray, q: float) -> TensorDataset:
    ...
    weights = torch.tensor(oracle_weights, dtype=torch.float32).unsqueeze(1)
    H1 = Y0
...
def train_outer_policies(train_loader: DataLoader, val_loader: DataLoader, params: Dict[str, Any], phi_type: int = 1, model_type: str = "linear") -> Tuple[nn.Module, nn.Module, float]:
    if model_type == "linear":
        ...
        model_f1 = Policy_NN(1, hidden_dim=params.get('hidden_dim', 32), num_layers=params.get('num_layers', 2)).to(device)
        model_f2 = Policy_NN(3, hidden_dim=params.get('hidden_dim', 32), num_layers=params.get('num_layers', 2)).to(device)
    else:
...
def optimize_outer_hyperparams(df_train: pd.DataFrame, oracle_train: np.ndarray, df_val: pd.DataFrame, oracle_val: np.ndarray, q: float, n_trials: int = 10, epochs: int = 150, phi_type: int = 1, model_type: str = "linear") -> Dict[str, Any]:
    ...
    def objective(trial):
        params = {'lr': trial.suggest_float('lr', 1e-4, 5e-2, log=True), 'l2': trial.suggest_float('l2', 1e-6, 1e-3, log=True), 'epochs': epochs}
        if model_type == "nn":
