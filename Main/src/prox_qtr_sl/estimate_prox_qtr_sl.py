...
def train_policy_prox_qtr_sl(n_train=2000, seed=20026, K_folds=2, max_alt_iters=10, tau=0.5, phi_type=1, model_type="linear", save_models=False):
    ...
    # === 2.5 Weight Trimming (Causal Inference Stability) ===
    # Clip extreme outlier weights generated by ill-posed Fredholm integrals
    q22_train_lower = np.percentile(q22_train_oof, 1)
    q22_train_upper = np.percentile(q22_train_oof, 99)
    q22_train_oof = np.clip(q22_train_oof, q22_train_lower, q22_train_upper)
    
    q22_val_lower = np.percentile(q22_val_preds, 1)
    q22_val_upper = np.percentile(q22_val_preds, 99)
    q22_val_preds = np.clip(q22_val_preds, q22_val_lower, q22_val_upper)
