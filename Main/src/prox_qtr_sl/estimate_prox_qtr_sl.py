...
def save_trained_models(f1, f2, best_params, n_train, tau, phi_type, model_type, seed, df_train, mmr_loss, q22_output_bound=5.0):
    ...
    for a1 in [1, -1]:
        for a2 in [1, -1]:
            ...
            _, final_q22_model, final_params = estimate_nuisance(
                sub_train_full, sub_val_full, a1, a2, n_trials=3,
...
def train_policy_prox_qtr_sl(n_train=2000, seed=20026, K_folds=2, max_alt_iters=30, tau=0.5, phi_type=1, model_type="nn", save_models=False, dgp="S1", mmr_loss="V_statistic", q22_output_bound=5.0):
    ...
    if save_models: # default: False
        save_trained_models(f1, f2, best_params, n_train, tau, phi_type, model_type, seed, df_train, mmr_loss, q22_output_bound=q22_output_bound)
    
    return f1, f2, q_current, best_sv

def train_policy_prox_qtr_no_cf(n_train=2000, seed=20026, max_alt_iters=30, tau=0.5, phi_type=1, model_type="nn", save_models=False, dgp="S1", mmr_loss="V_statistic", q22_output_bound=5.0):
    """
    ...
    for a1 in [1, -1]:
        for a2 in [1, -1]:
            ...
            predict_q22_fn, _, _ = estimate_nuisance(
                sub_train_full, sub_val_full, a1, a2, n_trials=10,
                mmr_loss_type=mmr_loss, q22_output_bound=q22_output_bound
            )
