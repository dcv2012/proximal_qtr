...
To prove Fisher consistency, we need the following lemma from (\textbf{Laha2021}): 
...
\begin{thm}[Fisher consistency]
    ...
    (b) (regret bound restriction) Also suppose assumption \ref{ass21} holds and $\psi(x,y)=\phi(x)\phi(y)$ satisfies condition \ref{cond2laha}, then  $SV^* - SV(f_1,f_2;q) \leq {(SV_\psi^* - SV_\psi(f_1, f_2;q))}/{(C_\phi^2/4)}$, for constant $C_\phi$ that satisfies condition \ref{cond2laha}.
    \label{fishercon}
\end{thm}
...
\begin{proof}
    ...
    \end{align}
    Under the same identifiable assumption in theory \ref{thm2}(b), we can further identify $SQ_1$ and $SQ_2$ as:
    \begin{align}
    ...
    \begin{lem}[]
        Suppose assumptions \ref{ass10}-\ref{ass14}, \ref{ass16}, \ref{ass18} and \ref{ass21} hold, and $\psi(x,y)=\phi(x)\phi(y)$ satisfies condition \ref{cond2laha}. Then for a fixed q, $(\tilde f_1,\tilde f_2) \in \bar{\mathcal{F}}$ satisfy $SV_\psi(\tilde f_1,\tilde f_2)=\sup\limits_{(f_1,f_2)\in \mathcal{F}} SV_\psi(f_1,f_2)$ if
        \[
    ...
    \begin{lem}[\textbf{Lemma H.2} in \textbf{Laha2021}]
        ...
        \label{lemh2laha}
    \end{lem}
    And the following lemma show the essence of treatment bridge functions that are helpful in the following proof. It can be shown directly from assumption \ref{ass18}.
    \begin{lem}[]
    ...
    The next step is to lower bound the two term in \eqref{fcpf1}.
