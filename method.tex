...
\begin{align}
   \hat Q_\tau(Y_2(d_1,d_2))= \argmin_{q} \frac{1}{n}\sum_{i=1}^n\Big\{\hat q_{22}(\bar Z_{2,i},\bar Y_{1,i},\bar A_{2,i})I(d_1(Y_{0,i})=A_{1,i})I(d_2(\bar Y_{1,i},A_{1，i})=A_{2,i})\cdot \rho_\tau(Y_{2,i}-q)  \Big\} ,
\end{align}
...
Suggested by \textbf{(Laha2021)}, now we can utilize a specific surrogate loss $\psi(x,y) = \phi(x)\phi(y)$, in which 
$\phi(x)$ can be 
\begin{itemize}
...
In addition, we also implement the cross-fitting method to estimate the bridge function $q$, according to the following procedure:
\begin{enumerate}
    ...
    \item For each $k=1,\ldots,K$, use the data $I^{(-k)}:=\mathcal{O}\textbackslash I_k$ to estimate (train) the outcome confounding bridge function $\hat q_{22}^{(-k)}$ and calculate the value of $\hat q_{22}^{(-k)}$ on $I_k$.
...
\begin{itemize}
    ...
    For each model setting, run a Monte Carlo simulation of 100 repos and record 
    \begin{itemize}
        \item the true quantile
        \item mean quantile estimated by the model
        \item standard deviation
        \item true regret
        \item RMSE
    \end{itemize}
    in a separate file under the directory Main/results/mc\_res.
    The saved files should be named by their parameters.
    For training sample size $n$ = 500,1000,5000,10000, plot the values of true regret and RMSE (in two plots) of each model as $n$ increases.
    When $n$=10000, summarize the mean true quantile, standard deviation, regret, and RMSE for these eight models in a three-line table.
    ...
    For SRA estimator, denote $X_1^{SRA} = \{Y_0,Z_1,W_1\}, X_2^{SRA} = \{\bar Y_1, \bar Z_2, \bar W_2, A_1\}$, and we have the SRA weight
    \[
        \hat{W}^{SRA} := \frac{1}{\hat \pi_1(A_1|X_1^{SRA})\hat \pi_2(A_2|X_2^{SRA})}
        = \frac{1}{\hat \pi_1(A_1|Y_0,Z_1,W_1)\hat \pi_2(A_2|\bar Y_1, \bar Z_2, W_2, A_1)}，
    \]
