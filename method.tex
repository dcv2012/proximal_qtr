...
In this section, we propose a quantile-optimal DTR estimator smoothed by surrogate loss functions instead of indicator functions. 
For simplicity, we only study the situation of $K=2$ and $A_k \in \{-1,1\}$ for each $k=1,2$ (in the setting of $A_k \in \{0,1\}$, we can use a map $A_k'=g(A_k) = 2A_k-1$ to reallocate treatments to the set of $\{-1,1\}$). The settings of the parameters are in line with section 3. 
The process of solving the quantile-optimal treatment regime $d^*$ is known as a bi-level optimization problem that allows us to consider the following optimization framework:
\begin{itemize}
    ...
    \Longleftrightarrow \quad & \mathbb{E}\big[\tau-I\{Y_2(d_1,d_2) < Q_\tau(Y_2(d_1,d_2))\} \big]= 0 \notag
    \end{align}
    \item Outer level optimization: fixing $Q_\tau(Y_2(d_1,d_2))$, then find the quantile-optimal treatment regime  
    \begin{align}
...
In general, we have three steps to do with policy learning.
First, we need to pre-estimate the nuisance function -- treatment bridge function $\hat q_{22}(\bar Z_2,\bar Y_1, \bar A_2)$ according to \eqref{est}.
Second, we do the inner level optimization to find the argmin quantile value $q$ when fixing the policies $d_1$ and $d_2$.
Last, we do the outer level optimization by fixing $q$ and introducing a surrogate loss function 
$\psi$ to find the optimal policies.


\subsection{Pre-estimation of the treatment bridge function}
Before handling the target loss function, we need to estimate the nuisance parameter of the (non-)parametric treatment bridge function $q_{22}(\bar Z_{2}, \bar Y_{1}, \bar A_{2})$.
We adopt the DeepMMR method by (Zhang2025) to estimate it.
Since they only provide an estimation method for single-stage, we reformulate it as a two-stage estimation method for estimating the treatment bridge function $q_{22}(\bar Z_{2}, \bar Y_{1}, \bar A_{2})$.
...
\begin{align}
    & \hat q_{11}=\argmin_{q_{11}} \frac{1}{n^2}\sum_{i,j=1}^n (I(A_{1,i}=a_{1,i})q_{11,i}-1)(I(A_{1,j}=a_{1,j})q_{11,j}-1)\cdot k_{1,ij} + \lambda_1||q_{11}||_2^2, \\
    & \hat q_{22}=\argmin_{q_{22}} \frac{1}{n^2}\sum_{i,j=1}^n (I(A_{2,i}=a_{1,i})q_{22,i}-\hat q_{11,i})(I(A_{2,j}=a_{2,j})q_{22,j}-\hat q_{11,j})\cdot k_{2,ij} + \lambda_2||q_{22}||_2^2,
\end{align}
...
We now consider the optimization problem. In this section, we first address the inner level optimization in \eqref{inner}.
Based on \eqref{est}, the quantile-value function can be estimated by
\begin{align}
   Q_\tau(Y_2(d_1,d_2))= \argmin_{q} \frac{1}{n}\sum_{i=1}^n\Big\{\hat q_{22}(\bar Z_{2,i},\bar Y_{1,i},\bar A_{2,i})I(d_1(Y_{0,i})=A_{1,i})I(d_2(\bar Y_{1,i},A_{1，i})=A_{2,i})\cdot \rho_\tau(Y_{2,i}-q)  \Big\} ,
\end{align}
which can be solved by sorting methods or quantile regression for given $d_1$,$d_2$.


\subsection{Outer level optimization}
Motivated by (\textbf{Laha2021}), we consider solving the origin optimization problem as a weighted classification problem using a surrogate loss to smooth the indicator function of $d$. 
For a fixed $q$, consider an alternative form of targeted survival function 
\begin{align}
...
\begin{rem}[]
    It is essential to notice that the historical information set $H_t$ relies on the potential outcomes of previous stages under the proximal setting (Y.Gao, J.Zhang), which is different to the setting of no unmeasured confounder assumption (NUCA).
...
An additional assumption is required for smooth techniques in the following discussion:
\begin{ass}[Positivity of treatment bridge function]
    For $k=1,2$, there exists $\epsilon_k >0$ such that $q_{kk}(\cdot) > \epsilon_k >0$.
    \label{ass21}
\end{ass}
And the following lemma from (\textbf{Laha2021}) is helpful to construct a surrogate loss function: 
...
It is essential to prove the following Fisher consistency:
...
Now we can utilize a specific surrogate loss $\psi(x,y) = \phi(x)\phi(y)$ and $\phi(x)=1+ \frac{2}{\pi}\arctan{(\frac{\pi x}{2})}$, suggested by \textbf{(Laha2021)}.
...
We also apply the cross-fitting technique to remove the bias from estimating the nuisance function $\hat q_{22}$. For simplicity, we denote $q_{22}(\bar Z_2, \bar Y_1, \bar A_2)$ by $q_{22}$.
In details, we implement cross-fitting according to following procedure:
\begin{enumerate}
    \item we randomly split data $\mathcal{O}$ into $K$ non-intersecting folds, denoted as $I_1, \dots, I_K$ and $\bigcup_{k=1}^K I_k = \mathcal{O}$.
    \item For each $k=1,\ldots,K$, use the data $I^{(-k)}:=\mathcal{O}\textbackslash I_k$ to estimate the outcome confounding bridge function $\hat q_{22}^{(-k)}$ and calculate the value of $\hat q_{22}^{(-k)}$ on $I_k$.
    \item For each $k=1,\ldots,K$, on the $k-$th fold $I_k$, we obtain 
    \begin{align*}
        \hat{SV}_{\psi,-k}(f_1,f_2;q) := \mathbb{P}_{n,k} \big[I(Y_2>q)\hat q_{22}^{(-k)}\psi(A_1f_1(H_1),A_2f_2(H_2)) \big],
    \end{align*}
    where $\mathbb{P}_{n,k}$ denotes the empirical average over the $k-$th fold.
    Then we can define the ($k-$average) empirical $\psi-$survival value function as 
    \begin{align*}
        \hat{SV}_\psi(f_1,f_2;q) = \frac{1}{K}\sum_{k=1}^K \hat SV_{\psi,-k}(f_1,f_2;q) = \frac{1}{K} \sum_{k=1}^K \mathbb{P}_{n,k} \big[I(Y_2>q)\hat q_{22}^{(-k)}\psi(A_1f_1(H_1),A_2f_2(H_2)) \big],
    \end{align*}
\end{enumerate}

Under these settings, the ultimate learning goal is to find $(\hat f_{n,1}, \hat f_{n,2})$ such that 
\begin{align}
        (\hat f_{n,1},\hat f_{n,2}) =& \argmax_{(f_1,f_2) \in \mathcal{U}_n}\hat{SV}_\psi(f_1,f_2;q) \notag\\
        =& \argmax_{(f_1,f_2) \in \mathcal{U}_n}\frac{1}{K} \sum_{k=1}^K \mathbb{P}_{n,k} \big[I(Y_2>q)\hat q_{22}^{(-k)}\psi(A_1f_1(H_1),A_2f_2(H_2)) \big],
\end{align}
...
Similar to (Ying2023), we generate a two stage longitudinal data of 11 variables: $(Y_0,U_0,A_1,Z_1,W_1,Y_1,U_1,A_2,W_2,Z_2,Y_2)$. 
...
\begin{align*}
    P(A_1|Y_0,U_0) = \frac{1}{1+\exp\{-(-0.5 - 0.35Y_0 + 0.35U_0)\}},
\end{align*}
...
\begin{align*}
    P(A_2|A_1,\bar Y_1,\bar U_1) = \frac{1}{1+\exp\{-(-0.25+0.25A_1-0.35Y_0+0.35U_0-0.35Y_1+0.35U_1)\}},
\end{align*}
...
\begin{itemize}
    \item Performance analysis: investigate how the model’s performance (runtime, value of estimator \eqref{est}) varies under different experimental settings, which include training (validation) set size, $\tau$ value, $\phi$ types and model $f_1,f_2$ type. The list of different experimental settings:
    \begin{itemize}
        \item training set size: 500, 2000, 5000
        \item $\tau$ value: 0.25, 0.5, 0.75
        \item $\phi$ types: 1,2,3,4
        \item model type: linear, neural network.
    \end{itemize} 
        
    \item Offline evaluation: using trained model to evaluate (inference) test data, calculating its quantile-value function and survival value function. 
    
    \item Monte Carlo simulation: after getting estimated $\hat f_{n,1}, \hat f_{n,2}$, we can get $\hat d_{n,1}= sign (\hat f_{n,1}), \hat d_{n,2} = sign(\hat f_{n,2}).$
    ...
    \item Parameters analysis: generate 8 modelss corresponding to the following settings: $\tau$=0.5, $\phi$ types: 1,2,3,4 and model type: linear, nerual network.
    For each model setting, run Monte Carlo simulation of 100 repos and record 
    \begin{itemize}
        \item the oracle true quantile
        \item mean true quantile estimated by the model
        \item standard deviation
        \item true regret
        \item RMSE
    \end{itemize}
    in a separate file under the directory Main/results/mc\_res, which should be named by their parameters.
    For trainning sample size $n$ = 500,1000,5000,10000, plot the values of true regret and RMSE (in two plots) of each model as n increases.
    When $n$=10000, summarize the the mean true quantile, standard deviation, regret, and RMSE for these eight models in a three-line table.
