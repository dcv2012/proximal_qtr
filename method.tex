...
\begin{algorithm}[htb]
    ...
    \For{$k=1,2,\cdots,$}
    {
        ...
        \end{flalign*} \\
        \If{$|\hat{SV}_{\Phi}(\hat{f}_{n,1}^{(k)},\hat{f}_{n,2}^{(k)};m^{(k)};{h_n}) - (1-\tau)| \leq \varepsilon_n$}{\textbf{break}\;}
        \ElseIf{$\hat{SV}_{\Phi}(\hat{f}_{n,1}^{(k)},\hat{f}_{n,2}^{(k)};m^{(k)};{h_n}) \geq (1-\tau)$}
...
In this section, we summarize the complete learning process of our method, Prox\_qtr\_sl, for estimating the optimal policies $(d_1^*, d_2^*)$ in Algorithm \ref{alg1}. $\hat{SV}_{\Phi}(\hat{f}_{n,1}^{(k)},\hat{f}_{n,2}^{(k)};m^{(k)};{h_n})$ is defined as
\begin{align}
    \hat{SV}_{\Phi}(\hat{f}_{n,1}^{(k)},\hat{f}_{n,2}^{(k)};m^{(k)};{h_n}) = \frac{1}{n} \sum_{i=1}^n  \big[I(Y_{2,i}>m^{(k)}) \cdot \hat q_{22}(\bar Z_{2,i}, \bar Y_{1,i}, \bar A_{2,i}) \Phi\{A_{1,i}f_1(H_{1,i})/h_n\}\Phi\{A_{2,i}f_2(H_{2,i})/h_n\} \big], \notag
\end{align}
...
In this section, we discuss the risk bound with surrogate form, i.e., the $\psi-$regret bound.
In fact, it suffices to bound the $\psi-$regret according to Fisher consistency.
Without loss of generality, we assume that the optimal $q^*$ has already been found, so that we can proceed with the following discussion with fixed $q = q*$. 

To this end, we will re-clarify some certain notations that may be referred to in the following discussion:
\begin{align}
