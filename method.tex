...
\begin{algorithm}[htb]
    ...
    \Input{Observed data $\{\bar Y_{2,i},\bar Z_{2,i},\bar A_{2,i}, \bar W_{2,i}\}^n_{i=1}$, quantile level $\tau$, initial interval lower bound $l^{(0)}$ and upper bound $u^{(0)}$, tolerance threshold $\varepsilon_n, \kappa_n$}
    ...
    \For{$k=1,2,\cdots,$}
    {
        ...
        \If{$|\hat{SV}(\hat{d}_{n,1},\hat{d}_{n,2};m^{(k)}) - (1-\tau)| < \varepsilon_n$}{\textbf{break}\;}
        \ElseIf{$\hat{SV}(\hat{d}_{n,1},\hat{d}_{n,2};m^{(k)}) - (1-\tau) > \varepsilon_n$}
...
In this section, we summarize the complete learning process of our method, named Prox\_qtr\_sl, to estimate the optimal policies $(d_1^*,d_2^*)$ in the algorithm \ref{alg1}. 
